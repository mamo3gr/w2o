...
# This script is based on above article:
    ...
    def fetch_img_direct(self, url):
        ...
        try:
            with urlopen(req, timeout=3) as p:
                ...
                structured_page = BeautifulSoup(page_b_content.decode('UTF-8'),
                                                'html.parser')
                img_link_elems = structured_page.find_all('img')
                img_urls = [e.get('src') for e in img_link_elems
...
def url_brancher(word: str, n_images: int):
    """
    ...
    返り値:
        url_list : yahoo画像検索画面のurlのリストです。
    """
    constant =\
        "https://search.yahoo.co.jp/image/search?p={{word}}&n={num}".format(
        word=word, num=n_images)
    ...
    for i in range(len(values[0])):
        for j in range(len(values[1])):
            for k in range(len(values[2])):
                url_list.append(
                    constant
                    + "&dim={}".format(values[0][i])
                    + "&imc={}".format(values[1][j])
                    + "&ctype={}".format(values[2][k]))
    return url_list


def main(word, data_dir: str, n_images: int):
    """
    ...
    yahoo_url_list = url_brancher(word, n_images)
